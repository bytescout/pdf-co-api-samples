...
##### **ReplaceTextWithImageFromUploadedFile.js:**
...
function replaceImageFromPdf(apiKey, uploadedFileUrl, password, destinationFile) {
    ...
    var queryPath = `/v1/pdf/edit/replace-text-with-image?name=${path.basename(destinationFile)}&password=${password}&url=${uploadedFileUrl}&async=True&searchString=The most /creativecommons.org/licenses/by-sa/3.0/ feature of&replaceImage=https://bytescout-com.s3.amazonaws.com/files/demo-files/cloud-api/image-to-pdf/image1.png text`;
    let reqOptions = {
        host: "api.pdf.co",
        path: encodeURI(queryPath),
        method: "GET",
